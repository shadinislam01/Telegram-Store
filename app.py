...
MONETAG_ZONE_ID = '10381248'

# Ad Types Configuration
AD_TYPES = {
    'monetag_interstitial': {
        'name': 'Quick Video',
        'description': 'Short 15-30 second video ads',
        'reward': 3,
        'cooldown': 15
    },
    'monetag_rewarded_interstitial': {
        'name': 'Premium Video',
        'description': '30-60 second premium video ads',
        'reward': 8,
        'cooldown': 30
    },
    'monetag_rewarded_popup': {
        'name': 'Instant Reward',
        'description': 'Quick popup ads for instant rewards',
        'reward': 5,
        'cooldown': 20
    }
}
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        cnt_rewarded_interstitial INTEGER DEFAULT 0,
        cnt_rewarded_popup INTEGER DEFAULT 0,
    ...
    defaults = [
        ('limit_interstitial', '100', 'Daily limit for Quick Video ads'),
        ('limit_rewarded_interstitial', '50', 'Daily limit for Premium Video ads'),
        ('limit_rewarded_popup', '80', 'Daily limit for Instant Reward ads'),
        ('reward_interstitial', '3', 'Credits for Quick Video ads'),
        ('reward_rewarded_interstitial', '8', 'Credits for Premium Video ads'),
        ('reward_rewarded_popup', '5', 'Credits for Instant Reward ads'),
...
def get_user(uid):
    ...
    if user['last_ad_date'] != today:
        conn.execute('''
            UPDATE users SET ads_today = 0, cnt_interstitial = 0,
                           cnt_rewarded_interstitial = 0, cnt_rewarded_popup = 0,
                           last_ad_date = ? WHERE user_id = ?
        ''', (today, uid))
        conn.commit()
        user = conn.execute('SELECT * FROM users WHERE user_id = ?', (uid,)).fetchone()
...
def earn():
    data = request.json
    uid = data.get('uid')
    ad_type = data.get('ad_type', 'monetag_interstitial')
    ...
    try:
        ...
        if user['last_ad_time']:
            ...
            ad_cooldown = AD_TYPES[ad_type]['cooldown']
            if time_diff < ad_cooldown:
                wait_time = ad_cooldown - time_diff
                return jsonify({
                    'success': False,
        ...
        config = get_config()
        limits = {
            'interstitial': int(config.get('limit_interstitial', '100')),
            'rewarded_interstitial': int(config.get('limit_rewarded_interstitial', '50')),
            'rewarded_popup': int(config.get('limit_rewarded_popup', '80'))
        }
        
        ad_columns = {
            'monetag_interstitial': 'cnt_interstitial',
            'monetag_rewarded_interstitial': 'cnt_rewarded_interstitial',
            'monetag_rewarded_popup': 'cnt_rewarded_popup'
        }
        
        ad_limit_key = {
            'monetag_interstitial': 'interstitial',
            'monetag_rewarded_interstitial': 'rewarded_interstitial',
            'monetag_rewarded_popup': 'rewarded_popup'
        }
        
        column = ad_columns[ad_type]
        limit_key = ad_limit_key[ad_type]
        
        if user[column] >= limits[limit_key]:
            return jsonify({
                'success': False,
                'message': f'Daily limit reached for {AD_TYPES[ad_type]["name"]}'
            })
        
        # Process reward
        reward = AD_TYPES[ad_type]['reward']
        today = datetime.now().strftime("%Y-%m-%d")
        ...
        conn.execute(f'''
            UPDATE users SET credits = credits + ?, total_earned = total_earned + ?,
                           ads_today = ads_today + 1, {column} = {column} + 1,
        ...
        conn.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (uid, f'📺 {AD_TYPES[ad_type]["name"]}', reward, date, 'earn'))
...
def admin_dashboard():
    ...
    stats = {
        ...
        'ads_today': conn.execute('SELECT SUM(ads_today) FROM users').fetchone()[0] or 0,
